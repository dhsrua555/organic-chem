...
import { molecule, tokensHTML, esc, getLang, onLang, store, tabsHTML, tabNow } from '../ui.js';
...
const STEP_KO = ['주 작용기', '주사슬', '번호', '접두사', '입체'];
...
export function mount(root, app) {
  ...
  function render() {
    ...
    const steps = [
      step(1, 'PRINCIPAL GROUP', '주 작용기 찾기', `<p>분자에 있는 작용기 중 <b>우선순위가 가장 높은 하나</b>가 이름 끝(접미사)을 차지합니다. 나머지는 모두 접두사가 됩니다.</p>
          ...
          <p>예시에서는 OH(알코올)뿐이므로 접미사 <code>-ol</code>. 전체 순위는 ‘우선순위’ 탭에 있습니다.</p>`, ex0),
      ...
      step(5, 'STEREO', '입체 표시 (E/Z · R/S · cis/trans)', `<p><b>E/Z</b> — 이중결합은 돌지 않아서 치환기 배치가 고정됩니다. 양 끝에서 CIP 우선순위가 높은 치환기끼리</p>
          ...
          <li>그림에서 쐐기(▲)는 앞으로, 빗금 쐐기는 뒤로 들어간 결합. H 가 앞으로 나와 있으면 보이는 방향을 거꾸로 읽습니다</li>
          <li>똑같이 생긴 두 가지가 한쪽은 R, 한쪽은 S 중심을 품으면 그 탄소는 <b>가짜 비대칭 중심</b> — 소문자 <b>r · s</b> 로 적습니다 (R 가지가 S 가지보다 높음)</li></ul>
    ...
    const prio = `<p class="tab-lead">접미사 우선순위 한눈에 — 위에 있을수록 이름 끝(접미사)을 차지합니다.</p>
    ...
    const stems = `<p class="tab-lead">탄소 수 → 어근</p>
    ...
    const benzH = `<p class="tab-lead">벤젠에 하나 붙으면 고유 이름이 모체가 됩니다. 눌러서 분자 조립에서 열기.</p>
    ...
    const koH = `<p class="tab-lead">대한화학회 표기 읽는 법</p>
    ...
    const y2013 = `<p class="tab-lead">IUPAC 2013 권고와 옛 교과서 규칙이 달라진 곳</p>
      <div class="rule-step" style="border-top:0;padding-top:0">
    ...
    root.innerHTML = `<section class="page rules">
      <div class="rules-hero">
        ...
        <p class="lead">IUPAC 이름은 규칙을 정해진 순서대로 적용하면 하나로 정해집니다. 예시 분자 ${ex.m.res.nameEn.replace(/.*/, s => `<code>${esc(s)}</code>`)} 로 다섯 단계를 한 단계씩 따라가 봅니다. 표와 규칙은 아래 탭에서 골라 보세요.</p>
      </div>
      ${tabsHTML('rules', [
        { id: 'steps', label: '다섯 단계', html: tabsHTML('rstep', steps.map((h, i) => ({ id: String(i + 1), label: `${i + 1} ${STEP_KO[i]}`, html: h + stepNav(i) }))) },
        { id: 'prio', label: '우선순위', html: prio },
        { id: 'stems', label: '어근', html: stems },
        { id: 'benz', label: '벤젠', html: benzH },
        { id: 'ko', label: '한글 이름', html: koH },
        { id: 'y2013', label: '2013 vs 교과서', html: y2013 }
      ], 'steps')}
    </section>`;
  ...
  function stepNav(i) {
    return `<div class="step-nav">${i > 0 ? `<button class="btn" type="button" data-goto="rstep:${i}">← ${i} ${STEP_KO[i - 1]}</button>` : '<span></span>'}${i < 4 ? `<button class="btn solid" type="button" data-goto="rstep:${i + 2}">다음: ${i + 2} ${STEP_KO[i + 1]} →</button>` : `<button class="btn solid" type="button" data-goto="rules:prio">우선순위 표 보기 →</button>`}</div>`;
  }
