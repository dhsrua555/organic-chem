...
import { entry, tokensHTML, esc, store, getLang, onLang, tabsHTML, drawMode } from '../ui.js';
import { commonName } from '../chem/common.js';
import { defineMissing } from '../chem/stereo.js';
import { rsCardHTML } from '../rsview.js';

export const EXAMPLES = {
  sn: [['C[C@@H](Br)CC', '(R)-2-브로모뷰테인'], ['CC(C)(C)Br', 'tert-뷰틸 브로마이드'], ['CCCBr', '1-브로모프로페인'], ['CC(C)C(C)Br', '2-브로모-3-메틸뷰테인 (자리옮김)'], ['BrCc1ccccc1', '벤질 브로마이드'],
  ...
  co: [['CCC=O', '프로판알'], ['CC(=O)c1ccccc1', '아세토페논'], ['O=C1CCCCC1', '사이클로헥산온'], ['CC(=O)C(C)(C)C', '피나콜론 (바이어–빌리거)'], ['CC=CC(C)=O', '펜트-3-엔-2-온 (짝지은 첨가)'], ['CCOC(C)=O', '아세트산 에틸'], ['CCC#N', '프로페인나이트릴'], ['OCCC(C)=O', '4-하이드록시뷰탄-2-온']],
...
const ROLE = { major: '주생성물', minor: '부생성물', side: '부산물' };
...
export function mount(root, app, params) {
  ...
  root.innerHTML = `<section class="page react">
    <div class="pg-head">
      ...
      <p class="lead" style="margin:0;max-width:62ch">기질과 시약을 선택하면 스미스 · 브루스 유기화학의 반응 규칙에 따라 주생성물 · 부생성물과 입체화학(R/S · cis/trans · E/Z)을 예측하고, 반응 메커니즘 · 선택성의 근거와 최근 연구 동향을 제시합니다.</p>
    </div>
    <div class="rx-grid">
      <div class="rx-left">
        <div class="panel ticks rx-sub">
          ...
          <div class="tools-row"><button class="btn" type="button" id="r-from">편집기의 구조 가져오기</button><button class="btn" type="button" id="r-edit">편집기에서 수정</button></div>
        ...
        <div class="panel rx-reag">
          <div class="rx-cats" role="tablist" aria-label="반응 분류">${CATS.map(c => `<button type="button" role="tab" data-cat="${c.id}"><b>${c.ko}</b><small>${c.sub}</small></button>`).join('')}</div>
  ...
  function paintResult() {
    ...
    if (!res.ok) {
      box.innerHTML = `<div class="panel ticks rx-scheme none"><p class="lbl">${esc(R.label)} · ${esc(R.note)}</p><p class="rx-none"><b>${esc(res.mech || '반응 없음')}</b></p><p>${esc(res.reason)}</p>
        <p class="hint">이 기질에 적용할 수 있는 시약은 목록에서 밝게 표시됩니다.</p></div>`;
      return;
    }
    ...
    box.innerHTML = `
      <div class="panel ticks rx-scheme${res.warn ? ' warn' : ''}">
        ...
        ${others.length ? `<div class="rx-minor"><p class="lbl">부생성물 · 부산물</p><div class="rx-prods small">${others.map(prodHTML).join('')}</div></div>` : ''}
      </div>
      ${tabsHTML('react', [
        res.steps && res.steps.length && { id: 'steps', label: '반응 메커니즘', html: `<div class="panel"><ol class="steps">${res.steps.map(s => `<li><div><span class="sk">${esc(s.t)}</span>${s.d}${s.mol ? `<div class="step-mol">${drawMolecule(s.mol, null, { mode: mode(), locants: false, chain: false, compact: true })}</div>` : ''}</div></li>`).join('')}</ol></div>` },
        (res.select && res.select.length || res.stereoLine) && { id: 'select', label: '선택성', n: (res.select || []).length + (res.stereoLine ? 1 : 0), html: `<div class="changes"><ul>${res.stereoLine ? `<li><b>R/S: ${esc(res.stereoLine)}</b></li>` : ''}${(res.select || []).map(s => `<li>${esc(s)}</li>`).join('')}</ul></div>` },
        { id: 'stereo', label: '입체화학', html: rsBlock(rsProd = major.find(p => p.name && ((p.name.rs && p.name.rs.size) || (p.name.pseudo && p.name.pseudo.size)))) },
        res.modern && res.modern.length && { id: 'modern', label: '최근 연구', n: res.modern.length, html: `<div class="modern panel">${res.modern.map(m => `<p><span class="ty">${esc(m.y)}</span>${esc(m.t)}</p>`).join('')}</div>` }
      ], 'steps')}
      <div class="tools-row"><button class="btn" type="button" id="r-take">주생성물로 다음 반응</button><button class="btn" type="button" id="r-build">주생성물을 편집기에서 열기</button><button class="btn" type="button" id="r-quiz">반응 연습 문제</button></div>`;
  ...
  let rsProd = null;
  const rsBlock = (p, sel) => p ? rsCardHTML(p.mol, p.name, { ko: getLang() === 'ko', racemic: true, sel }).replace('입체중심의 R/S · CIP 우선순위', '주생성물의 R/S · CIP 우선순위') : '';
  const placeholder = () => `<div class="panel rx-empty"><p class="lbl">예측 결과</p><p>시약을 선택하면 생성물이 여기에 표시됩니다.</p><p class="hint">밝게 표시된 시약은 이 기질과 반응할 수 있는 시약입니다. 흐리게 표시된 시약을 선택하면 반응이 일어나지 않는 이유를 설명합니다.</p></div>`;
  ...
  return {
    ...
    report: () => ({
      ...
      '반응': S.rid ? `${S.rid} (${((REACTIONS.find(x => x.id === S.rid) || {}).label || '').replace(/<[^>]+>/g, '')})` : '(선택 안 함)',
